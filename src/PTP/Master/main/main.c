...
static void render_screen_main(void)
{
    ...
    case LED_STATE_ACQUIRING:
         ...
         * "WAIT GPS" is the honest label; after that, a drop back to ACQUIRING
         * is a genuine re-acquisition of a clock that was once correct. */
        state_str = g_ever_locked ? "ACQUIRING" : "WAIT GPS";
        break;
    case LED_STATE_LOCKED:
    ...
    /* GPS sat indicator (line 4) — sentinel: GPS recently updated? */
    time_t now = time(NULL);
 ...
 *   - Verdict (EXCELLENT / GOOD / MARGINAL / POOR / UNUSABLE)

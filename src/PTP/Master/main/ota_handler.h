...
/* State flag so /api/ota/info can report "in progress". Updated only by the
 * OTA handler, read by the status handler. */
...
if (g_ota_in_progress) {
    ...
    /* Leave g_ota_in_progress = true so concurrent uploaders see "busy"
     * during the 1-second pre-reboot window. */

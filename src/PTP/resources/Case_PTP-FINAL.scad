...
09.Jul.2026 - Modified for PTP box by Cristiano Monteiro <cristianomonteiro@gmail.com>
...
//    } // Fin PCB 
    ...
    translate([PCBPosX,PCBPosY,FootHeight+(Thick/2)-0.5]){
...
//    }  
    ...
    // Lower right
    translate([PCBPosX-FeetDistBorder+92,PCBPosY+FeetDistBorder,Thick/2]){
        foot(FootDia,FootHole,FootHeight);
    }
    // Lower left
    translate([PCBPosX+FeetDistBorder+24.4,PCBPosY+FeetDistBorder,Thick/2]){
        foot(FootDia,FootHole,FootHeight);
        }
    // Upper left
    translate([PCBPosX+FeetDistBorder+24.4,PCBPosY+PCBWidth-FeetDistBorder,Thick/2]){
        foot(FootDia,FootHole,FootHeight);
        }        
        
    // Upper right    
    translate([PCBPosX-FeetDistBorder+92,PCBPosY+PCBWidth-FeetDistBorder,Thick/2]){
        foot(FootDia,FootHole,FootHeight);
    } 
...
// Code below is incorrect but produces right results. Explanation:
// OpenSCAD parameter order for cylinder() is:
//cylinder(h, r1, r2, center);
//When you mix named and unnamed arguments, the unnamed //positional argument (10) can bind unexpectedly.
//Effectively, OpenSCAD interprets this closer to:
//cylinder(h=10, r=6.5);
//NOT:
//cylinder(h=10, d=6.5);
//Because 6.5 becomes treated as a radius
// correct call should be: cylinder(h=10, d=Cdia, $fn=50); where Cdia is the actul diameter.
// XXX need to fix since a future OpenSCAD upgrade may break the assumption that leads to the correct figure being rendered
...
//                     <- Cutting shapes from here ->  
        // Panel
        SquareHole  (1,67,9.5,35,23,3); //(On/Off, Xpos,Ypos,Length,Width,Filet)
        // Panel holes
        // LB
        CylinderHole(1,69,6.5,3.5);
        // RB
        CylinderHole(1,99.4,6.5,3.5);
        // LT
        CylinderHole(1,69,35.18,3.5);
        // RT
        CylinderHole(1,99.4,35.18,3.5);
        //SquareHole  (1,40,20,15,10,1);
        //SquareHole  (1,60,20,15,10,1); 
        // Led 0
        //CylinderHole(1,43,26,3.1);//(On/Off, Xpos, Ypos, Diameter)
        SquareHole  (1,33.5,20,5.7,2.7,1);
        // Led 1
        //CylinderHole(1,63,26,3.1);
        SquareHole  (1,51.5,20,5.7,2.7,1);
        // Led 2
...
//                      <- Adding text from here ->          
        ...
        LText(1,12,30,"Arial Black",3.5,"PTP Grandmaster");
        //LText(1,23.5,10,"Icons",4,"");
...
//                     <- Cutting shapes from here ->  
        ...
        // Ethernet
        SquareHole(1,17,-1,19,23.5,1);
        // Other connectors
        SquareHole(1,15.4,-1,64,14.5,1);
        //                            <- To here -> 
           }
       }
...
rotate([0,0,180]){
        ...
        translate([-Thick*2+m/2,-Length+Thick,Thick+m/2])
        //Panel(Length,Width,Thick,Filet);
        BPanL();
        }
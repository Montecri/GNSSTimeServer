...
static size_t emit_token(const name_layer_t *layer, uint32_t index, char *dst)
{
                          ...
                          mac_hostname_format_t format)
{
    ...
    /* Step 4: render each chunk as a word and join with dashes. */
    char buf[MAC_HOSTNAME_MAX_LEN + 1];
    size_t written = 0;
    for (unsigned i = 0; i < N_LAYERS; ++i) {
        if (i) buf[written++] = '-';
        written += emit_token(&NAME_LAYERS[i], indexes[i], buf + written);
    }
    buf[written] = '\0';
